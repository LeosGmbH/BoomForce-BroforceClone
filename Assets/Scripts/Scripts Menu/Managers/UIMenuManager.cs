	...
	public class UIMenuManager : MonoBehaviour {
		...
		IEnumerator LoadAsynchronously(string sceneName){ // scene name is just the name of the current scene being loaded
			...
			while (!operation.isDone){
				...
				// When the async operation reports ready, activate immediately (no user input).
				if (operation.progress >= 0.9f){
					loadingBar.value = 1f;
					// Clear any prompt text to avoid showing a key prompt
					if (loadPromptText != null) loadPromptText.text = string.Empty;
